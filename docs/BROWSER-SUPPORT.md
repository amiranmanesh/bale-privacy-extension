...
| Firefox 121+                       | Gecko    | `…-firefox.zip`                   | installs cleanly, `web-ext lint` reports no errors |
...
- **Firefox** — `web-ext lint` reports zero errors, and real Firefox installs
  the package as a temporary add-on without complaint. The blur itself was not
  re-verified there, because that needs a logged-in Bale session in a second
  browser; the rendering path is the same stylesheet.
